...
enum GameStage { PLAY, DEAD, WIN, COMPLETE, GAMEOVER, SCREENSAVER };
GameStage stage;

// UNO-style macros (type agnostic)
#ifndef MAX
...
float time_spent_at_same_angle = 0;

uint8_t controllerMAC[] = {0x08, 0xF9, 0xE0, 0x63, 0xE4, 0x43};
// LED setup
#define NUM_LEDS 144
...
#define BRIGHTNESS                                                             \
  150 // Use a lower value for lower current power supplies(<2 amps)
#define DIRECTION 1 // 0 = right to left, 1 = left to right
#define MIN_REDRAW_INTERVAL                                                    \
  16                   // Min redraw interval (ms) 33 = 30fps / 16 = 63fps
...
#define LED_TYPE                                                               \
  WS2811 // type of LED strip to use(APA102 - DotStar, WS2811 - NeoPixel) For
         // Neopixels, uncomment line #108 and comment out line #106
...
// char* stage;                       // what stage the game is at
// (PLAY/DEAD/WIN/GAMEOVER)
long stageStartTime; // Stores the time the stage changed for stages that are
                     // time based
...
// POOLS
Enemy enemyPool[10] = {Enemy(), Enemy(), Enemy(), Enemy(), Enemy(),
...
Conveyor conveyorPool[2] = {Conveyor(), Conveyor()};
...
LivesPacket livesPacket;
void OnDataRecv(uint8_t *mac, uint8_t *data, uint8_t len) {
  memcpy(&packet, data, sizeof(packet));

  joystickWobble = packet.wobble;

  float tilt = constrain(packet.tilt, -90, 90);

  // ---- DEADZONE ----
  if (abs(tilt) < JOYSTICK_DEADZONE) {
    joystickTilt = 0;
    lastTiltAngle = tilt;
    tiltHoldStart = millis();
    return;
  }

  // ---- DETECT CHANGE IN ANGLE ----
  // If tilt changes enough, reset hold timer
  if (abs(tilt - lastTiltAngle) > 3) { // 3 degree tolerance removes IMU noise
    tiltHoldStart = millis();
    lastTiltAngle = tilt;
  }

  // ---- TIME HELD AT SAME ANGLE ----
  time_spent_at_same_angle = (millis() - tiltHoldStart) / 1000.0; // seconds

  // ---- LOG ACCELERATION ----
  float gain = log10(time_spent_at_same_angle + 0.1);

  // Prevent negative or insane gain values
  // gain = constrain(gain, 0.0, 2.0);

  // ---- FINAL OUTPUT ----
  joystickTilt = constrain(2 * tilt * gain, -90, 90);

  // Optional: invert direction if needed
  if (DIRECTION) {
...
void setup() {
  Serial.begin(115200);
  WiFi.mode(WIFI_STA);
  WiFi.disconnect(); // for safety

  Serial.print("My MAC:");
  Serial.println(WiFi.macAddress());

  if (esp_now_init() != 0) {
    Serial.println("ESP-NOW INIIT FAILED");
    return; // start again
  }
  ...
  esp_now_add_peer(controllerMAC, ESP_NOW_ROLE_CONTROLLER, 1, NULL, 0);
...
void tickEnemies() {
  for (int i = 0; i < enemyCount; i++) {
    if (enemyPool[i].Alive()) {
      ...
      // Hit attack?
      if (attacking) {
        if (enemyPool[i]._pos > playerPosition - (ATTACK_WIDTH / 2) &&
            enemyPool[i]._pos < playerPosition + (ATTACK_WIDTH / 2)) {
          enemyPool[i].Kill();
        }
      }
      ...
      // Hit player?
      if ((enemyPool[i].playerSide == 1 &&
           enemyPool[i]._pos <= playerPosition) ||
          (enemyPool[i].playerSide == -1 &&
           enemyPool[i]._pos >= playerPosition)) {
        die();
        return;
      }
...
void tickBoss() {
  ...
  if (boss.Alive()) {
    boss._ticks++;
    for (int i = getLED(boss._pos - BOSS_WIDTH / 2);
         i <= getLED(boss._pos + BOSS_WIDTH / 2); i++) {
      leds[i] = CRGB::DarkRed;
      leds[i] %= 100;
    }
    // CHECK COLLISION
    ...
    // CHECK FOR ATTACK
    if (attacking) {
      if ((getLED(playerPosition + (ATTACK_WIDTH / 2)) >=
               getLED(boss._pos - BOSS_WIDTH / 2) &&
           getLED(playerPosition + (ATTACK_WIDTH / 2)) <=
               getLED(boss._pos + BOSS_WIDTH / 2)) ||
          (getLED(playerPosition - (ATTACK_WIDTH / 2)) <=
               getLED(boss._pos + BOSS_WIDTH / 2) &&
           getLED(playerPosition - (ATTACK_WIDTH / 2)) >=
...
void drawPlayer() { leds[getLED(playerPosition)] = CRGB(0, 255, 0); }
...
void updateLives(){
    ...
    esp_now_send(controllerMAC, (uint8_t *)&livesPacket, sizeof(livesPacket));
