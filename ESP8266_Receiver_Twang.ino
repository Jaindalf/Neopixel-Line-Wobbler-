...
// POOLS
int lifeLEDs[3] = {5,4,13};  //D1,D2,D7
Enemy enemyPool[10] = {
...
void updateJoystickTilt()
{
    // Read raw tilt from ESP-NOW packet
...
void OnDataRecv(uint8_t *mac,uint8_t *data,uint8_t len){
    ...
    //joystickTilt = constrain(packet.tilt, -90, 90);
    //joystickTilt = joystickTilt * 0.7 + (-constrain(packet.tilt,-90,90)) * 0.3;

   // joystickTilt=2*(constrain(packet.tilt,-90,90))*log10(time_spent_at_same_angle+0.1);
...
float tilt = constrain(packet.tilt, -90, 90);
 ...
 /**   if(packet.wobble>=ATTACK_THRESHOLD){
        attacking=1;
        attackMillis = millis();   
    }
 ...
 */
  /*  if(packet.tilt>0 &&packet.tilt<90){
        joystickTilt=-90;
    }
...
void getInput(){
    ...
    if(packet.wobble>0){
        joystickWobble=ATTACK_THRESHOLD;
    }
        */
}